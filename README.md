...
You can also pass explicit package source maps to `export()`. Packages listed
in `Local_Packages`, `URL_Packages`, or `GitHub_Packages` are installed from
that source and are not installed from CRAN by package name.

``` r
export(
  appdir = "C:/Users/Bizon/Documents/GitHub/AnalyticsShinyApp",
  destdir = "C:/Users/Bizon/Documents/GitHub/AnalyticsShinyApp/build/electron",
  app_name = "AnalyticsShinyApp",
  runtime_strategy = "system",
  GitHub_Packages = c(
    prettydoc = "AdrianAntico/prettydoc",
...
`Local_Packages` has the highest precedence, then `URL_Packages`, then
`GitHub_Packages`, then CRAN. For active local development, override the GitHub
source explicitly:
...
export(
  ...
  Local_Packages = c(
    ...
    AutoQuant = "C:/Users/Bizon/Documents/GitHub/AutoQuant"
  )
)
```
