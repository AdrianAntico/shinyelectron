...
<figure><picture><source srcset="../man/figures/update-flow-dark.svg" media="(prefers-color-scheme: dark)" /><img src="../man/figures/update-flow.svg" alt="Three panels. A version 1.0.0 app on the left requests latest.yml from a publish host in the middle, which lists a manifest and signed installers. An arrow on the right shows the app updated to version 1.1.0." /></picture><figcaption>Update flow: the installed app reads a manifest, fetches a signed artifact, then swaps in on the next quit.</figcaption></figure>

## Pick a publish host

GitHub Releases is the only provider currently supported. S3 and Generic HTTP are planned for a future release and are not yet wired into the build.

| Provider | Status | Best for | Requirements |
|----------|--------|----------|--------------|
| GitHub Releases | Supported | Open source projects | Public or private GitHub repo |
| S3 | Planned | Enterprise deployments | AWS S3 bucket |
| Generic HTTP | Planned | Custom infrastructure | Any HTTP server |
...
::: {.callout-important}
S3 is planned but not yet supported. `enable_auto_updates()` currently accepts only `"github"` as the provider, and S3 configuration in `_shinyelectron.yml` is not consumed by the build. This section documents the intended design for when S3 support is added.
:::
...
::: {.callout-important}
Generic HTTP is planned but not yet supported. `enable_auto_updates()` currently accepts only `"github"` as the provider, and generic HTTP configuration in `_shinyelectron.yml` is not consumed by the build. This section documents the intended design for when Generic HTTP support is added.
:::
...
- [Customizations](customizations.html): splash screen, system tray, application menu.
