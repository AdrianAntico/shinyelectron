...
On macOS these demo apps are ad-hoc signed but not notarized, so the first launch shows a warning that the developer cannot be verified. Right-click (or Control-click) the app, choose Open, and confirm. See the [Code Signing](code-signing.html) article for notarizing your own builds.
